...
对于本项目，$f_d = 3225$ Hz，$f_B = 4000$ Hz，低频噪声（0-3225 Hz）搬移后会落在$(f_d - f, f_d + f)$范围，其中$(0, 4000-3225) = (0, 775)$ Hz部分会进入基带，导致显著的性能下降。
...
设原始信号频谱为$X(f)$，包含载波附近信号$X_d(f)$（集中在$|f| \approx f_d = 3225$ Hz）和低频成分$X_L(f)$（$|f| < f_c$）。
...
\end{equation}
保留$f_d$附近的信号，去除$|f| < f_c = 3225$ Hz的低频干扰。
...
\textcolor{black}{关键问题}：原始信号$X_d(f)$主要在$f \approx f_d = 3225$ Hz附近，带宽约为200-300 Hz。
\begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{振荡幅度}：Q3 的震荡幅度更加显著，这主要是由于 IIR 滤波器的非线性相位失真
    \item \textbf{相位失真效应}：Q3 中不同频率分量经过 Butterworth 滤波器后产生不同的群延迟，导致波形产生更明显的振荡和失真
    \item \textbf{Q4 的特性}：虽然理想滤波器会产生 Gibbs 振铃，但其线性相位特性保持了各频率分量的时间关系，整体波形相对更接近真实基带信号
\end{itemize}
...
\textbf{高通滤波器（$f_c = 3225.1032$ Hz）：}

\begin{enumerate}
    \item 镜像频率：$f_c' = 22050/2 - 3225.1032 = 7799.8968$ Hz
    \item 设计低通滤波器（步骤同上）
    \item 对奇数索引系数取反
\end{enumerate}
...
结果（9个系数）：
\begin{align*}
b &= [8.459 \times 10^{-2}, -6.767 \times 10^{-1}, 2.369, \ldots] \\
a &= [1.000, -3.304, 5.500, -5.642, \ldots]
\end{align*}
