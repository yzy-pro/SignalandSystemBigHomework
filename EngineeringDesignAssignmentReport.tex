...
\textbf{步骤 4：频率偏差估计}
\begin{itemize}
    \item 在 10 Hz 到 10 kHz 范围内搜索峰值，排除直流分量
    \item 识别多个峰值：2773.79 Hz、2775.20 Hz、3223.75 Hz 等
    \item 通过对称峰值分析确定频谱对称轴
    \item 计算频率偏差：$f_d = \frac{2773.79 + 3223.75}{2} \approx 3000$ Hz
    \item 识别基带主要频率成分约为 225 Hz
\end{itemize}
...
通过对称峰值法确定真实的频率偏差：
\begin{itemize}
    \item \textbf{下边带峰值}：$f_1 = 2773.79$ Hz（幅度 0.003231）
    \item \textbf{上边带峰值}：$f_2 = 3223.75$ Hz（幅度 0.003222）
    \item \textbf{频率偏差（对称轴）}：$f_d = \frac{f_1 + f_2}{2} = 2998.77 \approx 3000$ Hz
    \item \textbf{基带频率成分}：$\Delta f = \frac{f_2 - f_1}{2} = 224.98 \approx 225$ Hz
\end{itemize}

\textbf{注意}：初步峰值搜索得到的 3225.16 Hz 是频谱中的一个能量峰值，对应原始信号在某频率的成分，而非频率偏差 $f_d$。正确的 $f_d$ 应通过频谱对称性确定，即 $f_d = f_c - \tilde{f_c} \approx 3000$ Hz。
...
\begin{itemize}
    ...
    \item 频谱呈现双边带结构，关于对称轴 $f_d \approx 3000$ Hz 分布
    \item 在对称轴两侧观察到幅度相近的峰值（2773.79 Hz 和 3223.75 Hz），反映了基带信号的能量分布
    \item 高频段（8 kHz 以上）幅度很小，主要是噪声
    \item 由于实信号FFT的共轭对称性，在 $f_s - f_d \approx 19050$ Hz 处存在镜像（超出Nyquist频率显示范围）
\end{itemize}
...
\begin{itemize}
    \item 频谱关于 $f_d \approx 3000$ Hz 呈现对称的双边带结构
    \item 下边带峰值：2773.79 Hz（幅度 0.003231），对应基带约 226 Hz 的分量
    \item 上边带峰值：3223.75 Hz（幅度 0.003222），对应基带约 224 Hz 的分量
    \item 两个峰值幅度几乎相等（差异仅 0.3\%），验证了 AM 信号的双边带对称性
    \item 符合错误解调后信号的理论预期：$X(f) = \frac{1}{2}[M(f-f_d) + M(f+f_d)]$，其中对称轴为 $f_d = f_c - \tilde{f_c} \approx 3000$ Hz
\end{itemize}
...
时域波形呈现出典型的调幅信号特征：
\begin{itemize}
    \item 信号呈现周期性振荡，主要振荡频率对应频谱中观察到的峰值（约 2773 Hz 和 3224 Hz）
    \item 振幅随时间缓慢变化，这是原始基带信号 $m(t)$ 的包络
    \item 幅度范围约在 $\pm 0.15$ 之间，信号较弱但清晰可辨
    \item 错误解调导致载波偏移 $f_d \approx 3000$ Hz，使得原本在真实载波 $f_c$ 处的信号被搬移到 $f_d$ 附近
\end{itemize}
...
通过频谱对称性分析确定频率偏差：
\begin{itemize}
    \item \textbf{对称峰值法}：观察到下边带峰值 2773.79 Hz 和上边带峰值 3223.75 Hz，幅度几乎相等
    \item \textbf{对称轴计算}：$f_d = \frac{2773.79 + 3223.75}{2} = 2998.77 \approx 3000$ Hz
    \item \textbf{物理意义}：$f_d = f_c - \tilde{f_c} \approx 3000$ Hz，即真实载波与错误解调载波的频率差
    \item \textbf{基带频率推断}：峰值对应基带约 $\Delta f = \frac{3223.75 - 2773.79}{2} = 224.98 \approx 225$ Hz 的分量
\end{itemize}
...
\textbf{(1) 对称峰值对（关于 $f_d \approx 3000$ Hz）：}
\begin{itemize}
    \item \textbf{下边带}：2773.79 Hz（幅度 0.003231）和 2775.20 Hz（幅度相近）
    \item \textbf{上边带}：3223.75 Hz（幅度 0.003222）
    \item \textbf{对称性验证}：
    \begin{align*}
...
\textbf{(2) 基带频率成分推断：}
\begin{itemize}
    \item \textbf{2757.57 Hz 峰值}：$3000 - 2757.57 = 242.43$ Hz，对应基带 242 Hz 分量
    \item \textbf{2773.79 Hz 峰值}：$3000 - 2773.79 = 226.21$ Hz，对应基带 226 Hz 分量
    \item \textbf{2775.20 Hz 峰值}：$3000 - 2775.20 = 224.80$ Hz，对应基带 225 Hz 分量
    \item \textbf{3223.75 Hz 峰值}：$3223.75 - 3000 = 223.75$ Hz，对应基带 224 Hz 分量（上边带）
\end{itemize}

这些频率（220-242 Hz）可能对应：
\begin{itemize}
    \item 语音基频的 2-3 次谐波（基频约 75-120 Hz）
    \item 语音的第一共振峰（F1），某些元音的 F1 在 200-300 Hz 范围
    \item 音乐中的低音音符（如 A3 = 220 Hz）
\end{itemize}

\textbf{(3) 高频镜像峰值：}
\begin{itemize}
    \item \textbf{18824.84 Hz}：这不是 $f_d$ 的镜像，而是高频能量峰值的镜像
    \item 实际上 $f_s - f_d = 22050 - 3000 = 19050$ Hz 才是 $f_d$ 的镜像位置
    \item 18824.84 Hz 对应的正频率峰值为：$22050 - 18824.84 = 3225.16$ Hz
\end{itemize}
...
\begin{itemize}
    \item 滤波器阶数：$N = 8$
    \item 截止频率：$f_c = 3000.1823$ Hz
    \item 归一化截止频率：$\omega_c = 2\pi f_c / f_s = 0.8549$ rad
...
\begin{align*}
    b &= [1.020 \times 10^{-1}, -8.162 \times 10^{-1}, 2.857, -5.713, 7.142, \\
      &\quad -5.713, 2.857, -8.162 \times 10^{-1}, 1.020 \times 10^{-1}] \\
    a &= [1.000, -3.630, 6.427, -6.942, 4.931, -2.335, \\
      &\quad 7.148 \times 10^{-1}, -1.288 \times 10^{-1}, 1.041 \times 10^{-2}]
\end{align*}
...
\textbf{高通滤波器传递函数}（$f_c = 3000.1823$ Hz）：
\begin{equation}
\begin{split}
    H_{HP}(z) &= \frac{1.020 \times 10^{-1} - 8.162 \times 10^{-1}z^{-1} + 2.857z^{-2}}{1.000 - 3.630z^{-1} + 6.427z^{-2}} \\
    &\quad \times \frac{- 5.713z^{-3} + 7.142z^{-4} - 5.713z^{-5}}{- 6.942z^{-3} + 4.931z^{-4} - 2.335z^{-5}} \\
    &\quad \times \frac{+ 2.857z^{-6} - 8.162 \times 10^{-1}z^{-7} + 1.020 \times 10^{-1}z^{-8}}{+ 7.148 \times 10^{-1}z^{-6} - 1.288 \times 10^{-1}z^{-7} + 1.041 \times 10^{-2}z^{-8}}
\end{split}
\end{equation}
...
高通滤波器的幅频响应特性：
\begin{itemize}
    \item 在截止频率 $f_c = 3000$ Hz 处，增益约为 $-3$ dB（0.707倍）
    \item 在低频段（$f < 1500$ Hz），信号被强烈衰减，增益接近 0
    \item 在高频段（$f > 4500$ Hz），增益接近 1，信号几乎无衰减
    \item 过渡带宽度约为 2000 Hz，斜率约为 $-48$ dB/octave（8阶滤波器）
\end{itemize}

\textbf{注意}：高通滤波器截止频率设置为 $f_c = 3000$ Hz，对应 Q1 确定的频率偏差 $f_d = f_c - \tilde{f_c} \approx 3000$ Hz，目的是保留频谱对称轴以上的信号。
...
\textbf{处理参数：}
\begin{itemize}
    \item 频率偏差（对称轴）：$f_d = 3000$ Hz
    \item 解调载波频率：$f_d = 3000$ Hz（用于频谱搬移）
    \item 采样频率：$f_s = 22050$ Hz
    \item 样本数：$N = 31265$
    \item 高通滤波器截止频率：$f_c = 3000$ Hz（8 阶 Butterworth）
    \item 低通滤波器截止频率：$f_B = 4000$ Hz（8 阶 Butterworth）
\end{itemize}

\textbf{注意}：此处使用的载波频率 $f_d = 3000$ Hz 是通过 Q1 对称峰值分析确定的频率偏差，而非简单的峰值搜索结果。
...
\begin{itemize}
    ...
    \item 主观听感：存在嗡嗡的背景噪声，但整体清晰
    \item 音频可正常播放，信号清晰，可以辨别出原音频内容为"Enjoy your spring break!"
\end{itemize}

\subsubsection{滤波顺序讨论}

\textbf{问题1：是否可以跳过高通滤波步骤？}

\textbf{理论分析：}

假设原始错误解调信号可以表示为：
\begin{equation}
x(t) = A + m(t)\cos(2\pi f_d t) + n_L(t) + n_H(t)
\end{equation}

其中：
\begin{itemize}
    \item $A$：直流分量
    \item $m(t)\cos(2\pi f_d t)$：主要信号（调制信号在$f_d$附近）
    \item $n_L(t)$：低频噪声（$f < f_d$）
    \item $n_H(t)$：高频噪声（$f > f_d$）
\end{itemize}
...
如果直接对$x(t)$进行载波相乘：
\begin{align}
x(t) \cdot \cos(2\pi f_d t) &= [A + m(t)\cos(2\pi f_d t) + n_L(t) + n_H(t)] \cdot \cos(2\pi f_d t) \\
&= A\cos(2\pi f_d t) + m(t)\cos^2(2\pi f_d t) + n_L(t)\cos(2\pi f_d t) \\
&\quad + n_H(t)\cos(2\pi f_d t)
\end{align}

利用三角恒等式$\cos^2(\theta) = \frac{1 + \cos(2\theta)}{2}$和$\cos\alpha\cos\beta = \frac{1}{2}[\cos(\alpha-\beta) + \cos(\alpha+\beta)]$：

\begin{align}
&= A\cos(2\pi f_d t) + \frac{m(t)}{2}[1 + \cos(4\pi f_d t)] \\
&\quad + \frac{1}{2}\{n_L(t)[\text{频谱在} \pm f_d] + n_H(t)[\text{频谱在} \pm f_d]\}
\end{align}
...
各分量的频谱分布：
\begin{enumerate}
    \item \textbf{直流项的贡献}：$A\cos(2\pi f_d t)$ 
    \begin{itemize}
        \item 频谱：$\delta(f - f_d) + \delta(f + f_d)$
        \item 位于$\pm f_d$，经低通滤波后被滤除（$f_d = 3225$ Hz $<$ $f_B = 4000$ Hz,实际$f_d < f_B$会部分保留）
        \item \textcolor{black}{问题}：在基带附近引入不需要的频率分量
    \end{itemize}
    
    \item \textbf{主信号的贡献}：$\frac{m(t)}{2}[1 + \cos(4\pi f_d t)]$
    \begin{itemize}
        \item 基带分量：$\frac{m(t)}{2}$（这是我们需要的）
        \item 高频分量：$\frac{m(t)}{2}\cos(4\pi f_d t)$（频谱在$\pm 2f_d$，被低通滤除）
    \end{itemize}
    
    \item \textbf{低频噪声的贡献}：$n_L(t)\cos(2\pi f_d t)$
    \begin{itemize}
        \item 原本$n_L(t)$频谱在$[0, f_d)$
        \item 相乘后频谱搬移到$[f_d - f_{n_L}, f_d + f_{n_L}]$
        \item \textcolor{black}{关键问题}：如果$f_d - f_{n_L} < f_B$，这部分噪声会进入基带！
    \end{itemize}
\end{enumerate}

对于本项目，$f_d = 3000$ Hz，$f_B = 4000$ Hz，低频噪声（0-3000 Hz）搬移后会落在$(f_d - f, f_d + f)$范围，其中$(0, 4000-3000) = (0, 1000)$ Hz部分会进入基带，导致显著的性能下降。

\textbf{结论：}高通滤波器不可跳过，它有效去除了$P_{n_L}$，使信噪比提升约$\Delta \text{SNR}$ dB。

\textbf{问题2：能否改变滤波顺序（先低通后高通）？}

\textbf{详细分析：}

设原始信号频谱为$X(f)$，包含以对称轴$f_d = 3000$ Hz为中心的双边带信号和低频成分$X_L(f)$（$|f| < f_c$）。

\textbf{方案A（正确）：高通 → 相乘 → 低通}

\textit{步骤1：}高通滤波
\begin{equation}
X_1(f) = X(f) \cdot H_h(f) \approx X_d(f) \quad \text{（去除}X_L(f)\text{）}
\end{equation}
保留$f_d$附近的信号，去除$|f| < f_c = 3000$ Hz的低频干扰。

\textit{步骤2：}载波相乘（时域相乘对应频域卷积）
\begin{equation}
x_2(t) = x_1(t) \cdot \cos(2\pi f_d t) \quad \Rightarrow \quad X_2(f) = \frac{1}{2}[X_1(f - f_d) + X_1(f + f_d)]
\end{equation}

由于$X_1(f) \approx X_d(f)$主要在$\pm f_d$附近，频谱搬移后：
\begin{itemize}
    \item \textbf{正频部分}：$X_d(f)$（在$+f_d$）搬移到$X_d(f - f_d)$（基带）和$X_d(f + f_d)$（$+2f_d$）
    \item \textbf{负频部分}：$X_d(-f)$（在$-f_d$）搬移到$X_d(-f - f_d)$（$-2f_d$）和$X_d(-f + f_d)$（基带）
\end{itemize}

基带分量为：
\begin{equation}
X_{2,\text{baseband}}(f) = \frac{1}{2}[X_d(f - f_d) + X_d(f + f_d)]|_{|f| < B}
\end{equation}

\textit{步骤3：}低通滤波（$f_B = 4000$ Hz）
\begin{equation}
Y(f) = X_2(f) \cdot H_l(f)
\end{equation}
保留基带分量（$|f| < f_B$），滤除$\pm 2f_d \approx \pm 6450$ Hz的高频分量。


\textbf{方案B（错误）：低通 → 相乘 → 高通}

\textit{步骤1：}低通滤波（$f_B = 4000$ Hz）
\begin{equation}
X_1(f) = X(f) \cdot H_l(f) = \begin{cases}
X(f), & |f| < f_B = 4000 \text{ Hz} \\
0, & |f| \geq f_B
\end{cases}
\end{equation}

\textcolor{black}{关键问题}：原始信号以$f_d = 3000$ Hz为对称轴，双边带分布，总带宽约$(f_d - f_B, f_d + f_B) = (-1000, 7000)$ Hz。
\begin{itemize}
    \item 如果信号完全在$(f_d - B, f_d + B) = (2925, 3525)$ Hz，则$f_d + B < f_B$，信号\textbf{完全保留}
    \item 但低频噪声$X_L(f)$（$f < f_c$）也被保留！
\end{itemize}

\textit{步骤2：}载波相乘
\begin{equation}
X_2(f) = \frac{1}{2}[X_1(f - f_d) + X_1(f + f_d)]
\end{equation}

$X_1(f)$包含：
\begin{itemize}
    \item $X_d(f)$（在$f_d$附近）→ 搬移到基带（$|f| < B$）和$2f_d$附近
    \item $X_L(f)$（在0附近）→ 搬移到$\pm f_d$附近
\end{itemize}

具体地：
\begin{align}
X_2(f) &= \frac{1}{2}[X_d(f - f_d) + X_d(f + f_d)] \quad \text{（有用信号在基带和}2f_d\text{）} \\
&\quad + \frac{1}{2}[X_L(f - f_d) + X_L(f + f_d)] \quad \text{（噪声在}\pm f_d\text{附近）}
\end{align}

\textit{步骤3：}高通滤波（$f_c = 3225$ Hz）
\begin{equation}
Y(f) = X_2(f) \cdot H_h(f) = \begin{cases}
0, & |f| < f_c \\
X_2(f), & |f| \geq f_c
\end{cases}
\end{equation}
...
\textcolor{black}{错误}：
\begin{itemize}
    \item 基带有用信号$X_d(f - f_d)$（$|f| < B \ll f_c$）被\textbf{完全滤除}！
    \item 仅保留$|f| \geq f_c = 3225$ Hz的部分，即：
    \begin{itemize}
        \item $\pm f_d$附近的噪声分量（从$X_L(f)$搬移而来）
        \item 可能部分保留$2f_d \approx 6450$ Hz的高频分量（若$2f_d - B > f_c$）
    \end{itemize}
\end{itemize}

\textbf{方案C（错误）：相乘 → 高通 → 低通}

\textit{步骤1：}载波相乘
\begin{equation}
X_1(f) = \frac{1}{2}[X(f - f_d) + X(f + f_d)]
\end{equation}

包含：
\begin{itemize}
    \item \textbf{基带}：$X_d(f - f_d) + X_d(f + f_d)$（有用信号，$|f| < B$）
    \item \textbf{$\pm f_d$附近}：$X_L(f \pm f_d)$（低频噪声搬移）
    \item \textbf{$\pm 2f_d$附近}：$X_d(f \pm 2f_d)$（高频分量）
\end{itemize}

\textit{步骤2：}高通滤波（$f_c = 3225$ Hz）
\begin{equation}
X_2(f) = X_1(f) \cdot H_h(f)
\end{equation}

\textcolor{black}{错误}：基带分量（$|f| < B < f_c$）被\textbf{完全滤除}！

仅保留：
\begin{align}
X_2(f) &= [X_L(f - f_d) + X_L(f + f_d)]_{|f| \geq f_c} \\
&\quad + [X_d(f - 2f_d) + X_d(f + 2f_d)]_{|f| \geq f_c}
\end{align}
...
保留$(f_c, f_B) = (3225, 4000)$ Hz区间：
\begin{itemize}
    \item 主要是从$X_L(f - f_d)$来的噪声（原本在0附近，搬移到$f_d$，满足$f_c < f_d < f_B$部分保留）
    \item \textbf{没有}有用的基带信号（已在步骤2被滤除）
\end{itemize}

\textbf{输出特性：}
\begin{equation}
Y(f) \approx X_L(f - f_d)|_{f \in (f_c, f_B)} \quad \text{（纯噪声）}
\end{equation}
...
\begin{table}[h]
\centering
\caption{三种滤波顺序的频谱分析对比}
\begin{tabular}{|c|c|c|c|}
\hline
\textbf{方案} & \textbf{基带信号} & \textbf{输出频率范围} & \textbf{结果} \\
\hline
A（正确） & 保留 & $|f| < f_B$ & 有用信号 \\
\hline
B（错误1） & 被高通滤除 & $|f| \geq f_c$ & 信号丢失 \\
\hline
C（错误2） & 被高通滤除 & $(f_c, f_B)$ & 纯噪声 \\
\hline
\end{tabular}
\end{table}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|l|l|l|}
        ...
        \hline
        频率偏差 & $f_d = f_c - \tilde{f_c}$ & 3000 Hz（对称轴） \\
        \hline
        高通截止频率 & $f_d$ & 3000 Hz \\
        \hline
        低通截止频率 & $f_B$ & 4000 Hz \\
...
\textbf{理想滤波器参数：}
\begin{itemize}
    \item 高通滤波器：$H_h(f) = \begin{cases} 0, & |f| < f_d = 3000 \text{ Hz} \\ 1, & |f| \geq f_d \end{cases}$
    \item 低通滤波器：$H_l(f) = \begin{cases} 1, & |f| \leq f_B = 4000 \text{ Hz} \\ 0, & |f| > f_B \end{cases}$
    \item 截止特性：理想砖墙型，无过渡带
\end{itemize}

\textbf{注意}：Q4 使用与 Q3 相同的频率偏差 $f_d = 3000$ Hz，但在频域实现理想滤波器。
...
对两种方法得到的时域信号进行定量比较：
\begin{itemize}
    \item \textbf{均方误差（MSE）}：$4.990 \times 10^{-2}$
    \item \textbf{均方根误差（RMSE）}：$2.234 \times 10^{-1}$
    \item \textbf{最大绝对差}：0.927
    \item \textbf{信号振幅}：Q3 最大值 0.110，Q4 最大值 0.086（相差约 28\%）
    \item \textbf{相关系数}：$0.402$（弱正相关）
    \item \textbf{归一化后相关系数}：$0.402$（与原始相关系数相同）
    \item \textbf{信噪比（SNR）}：$-20.76$ dB
\end{itemize}

\textbf{重要发现}：归一化后的相关系数与原始相关系数完全相同(0.402)，这验证了皮尔逊相关系数的一个重要性质：\textbf{对信号振幅不敏感}。相关系数的定义本身就包含了归一化（协方差除以标准差），因此它反映的是波形\textbf{形状}的相似性，而非振幅大小。这说明 Q3 和 Q4 之间 0.402 的低相关性完全是由于波形形状差异造成的，而非振幅差异。

\textbf{3. 相关性分析与振幅差异}

Q3 和 Q4 的解调结果相关系数为 $r = 0.402$（弱正相关），表明两种方法虽然都能完成解调任务，但在\textbf{波形形状}上存在显著差异。需要特别说明的是：

\begin{itemize}
    \item \textbf{振幅差异}：Q3 信号幅度(0.110)比 Q4(0.086)大约 28\%，这导致了较大的 MSE($4.99 \times 10^{-2}$)和较低的 SNR(-20.76 dB)
    \item \textbf{形状差异}：相关系数 0.402 反映的是波形形状的差异，而非振幅差异（相关系数对振幅缩放不敏感）
    \item \textbf{振幅差异的来源}：Q3 使用 IIR 滤波器可能在某些频段有增益，而 Q4 的理想滤波器增益严格为 0 或 1
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{时域特性差异}
    \begin{itemize}
        \item Q3：因果的时域响应，但由于非线性相位失真，不同频率分量的群延迟不同，导致波形产生显著的振荡和失真
        \item Q4：理想滤波器的 sinc 函数冲激响应产生 Gibbs 振铃，但线性相位特性保持了各频率分量的相对时间关系
    \end{itemize}
    
    \item \textbf{过渡带处理的影响}
    \begin{itemize}
        \item Q3：在截止频率 3000 Hz 附近有约 500 Hz 的过渡带，该区域内的频率分量被部分保留（衰减 3-40 dB）
        \item Q4：在 3000 Hz 处完全截断，过渡带宽度为 0，该频率以下完全抑制，以上完全保留
        \item 这种根本差异导致两种方法保留的频率成分不完全相同，即使使用相同的名义截止频率
    \end{itemize}
...
\textbf{结论：}相关系数 0.402 表明两种方法在信号细节上存在显著差异，主要来自滤波器实现方式的本质不同：

\begin{itemize}
    \item \textbf{非线性相位失真}：Q3 的 IIR 滤波器使不同频率分量经历不同的延迟，导致波形扭曲
    \item \textbf{Gibbs 振铃效应}：Q4 的理想滤波器在时域产生 sinc 函数振荡，影响瞬态响应
    \item \textbf{过渡带处理}：Q3 在 2500-3500 Hz 渐变衰减，Q4 在 3000 Hz 陡峭截断
    \item \textbf{频率分量保留}：两种方法实际保留的频率成分不完全相同
\end{itemize}

尽管相关性较低，但两种方法都成功完成了解调任务：
\begin{itemize}
    \item 两种方法都使用相同的参数 $f_d = 3000$ Hz（通过对称峰值法确定）
    \item 两种方法都成功提取了可辨识的音频信号
    \item 差异反映了理论（理想滤波器）与实践（可实现滤波器）之间的固有权衡
    \item 在实际应用中，应根据具体需求（实时性、相位线性、计算复杂度等）选择合适的方法
\end{itemize}

\textbf{4. 波形对比}

\textbf{(1) 全时域波形对比（图 \ref{fig:q4_full_comparison}）}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{codes/Q4/output/Q4_vs_Q3_full_comparison.png}
    \caption{Q3 与 Q4 解调信号全时域波形对比（全部 31265 个采样点）}
    \label{fig:q4_full_comparison}
\end{figure}

全时域波形对比图展示了整个信号的总体特征：
\begin{itemize}
    \item \textbf{整体包络一致}：两种方法得到的信号包络基本一致，说明都成功恢复了音频的幅度调制特性
    \item \textbf{周期性振荡}：两种信号都呈现明显的周期性振荡，这是原始音频信号的固有特性
    \item \textbf{峰值位置对应}：主要的峰值和谷值位置基本对应，验证了两种方法在宏观上的一致性
    \item \textbf{细节差异}：虽然整体趋势相同，但在细节上存在差异，这在局部放大图中更加明显
\end{itemize}

\textbf{(2) 局部细节对比（图 \ref{fig:q4_comparison}）}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{codes/Q4/output/Q4_vs_Q3_comparison.png}
    \caption{Q3 与 Q4 解调信号局部波形对比（前 2000 个采样点）}
    \label{fig:q4_comparison}
\end{figure}

从局部波形对比图可以更清楚地观察到细节差异：
\begin{itemize}
    \item \textbf{振荡幅度差异}：Q3（红色）的震荡幅度更加显著，这主要是由于 IIR 滤波器的非线性相位失真
    \item \textbf{相位失真效应}：Q3 中不同频率分量经过 Butterworth 滤波器后产生不同的群延迟，导致波形产生更明显的振荡和失真
    \item \textbf{波形同步性}：两种方法的波形在峰值位置基本对应，但具体形状有明显差异
    \item \textbf{Q4 的特性}：Q4（蓝色）虽然理想滤波器会产生 Gibbs 振铃，但其线性相位特性保持了各频率分量的时间关系，整体波形相对更平滑
\end{itemize}
...
\begin{itemize}
    ...
    \item 虽然理论上会产生振铃，但由于线性相位特性，整体波形失真相对较小
\end{itemize}

\subsubsection{IIR 滤波器的非线性相位失真}

Q3 中观察到的显著振荡主要来自 Butterworth 滤波器的非线性相位特性：

\textbf{群延迟失真：}

8阶 Butterworth 滤波器的相位响应是非线性的，不同频率的群延迟不同：
\begin{equation}
    \tau_g(f) = -\frac{d\phi(f)}{d\omega} = -\frac{1}{2\pi}\frac{d\phi(f)}{df}
\end{equation}
...
\begin{itemize}
    \item 高通滤波器（$f_c = 3225$ Hz）引入相位失真 $\phi_{HP}(f)$
    \item 低通滤波器（$f_c = 4000$ Hz）引入相位失真 $\phi_{LP}(f)$
    \item 总相位失真：$\phi_{total}(f) = \phi_{HP}(f) + \phi_{LP}(f)$
    \item 两个滤波器的非线性相位失真叠加，导致群延迟变化更加剧烈
\end{itemize}

\textbf{对信号的影响：}

\begin{itemize}
    \item \textbf{频率色散}：不同频率分量经历不同的延迟，原本同步的频率分量在时域上分离
    \item \textbf{波形失真}：基带信号的各频率成分相位关系被破坏，导致波形产生显著振荡
    \item \textbf{包络失真}：调制信号的包络形状发生改变，出现过冲和振荡
    \item \textbf{累积效应}：由于是级联结构，相位失真逐级累积，最终输出波形失真明显
\end{itemize}

\textbf{与 Q4 的对比：}

\begin{itemize}
    \item Q4 的理想滤波器虽然产生 Gibbs 振铃，但其相位响应是线性的（零相位或线性相位）
    \item 线性相位意味着所有频率分量经历相同的群延迟，只是整体平移，不改变相对时间关系
    \item 因此 Q4 能更好地保持原始信号的波形结构，虽有振铃但整体失真较小
    \item Q3 的非线性相位失真导致的波形变化比 Q4 的 Gibbs 振铃更加显著
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{频率选择性}：Butterworth 滤波器的渐变过渡带提供良好的阻带衰减
...
\begin{enumerate}
    ...
    \item \textbf{相位失真严重}：IIR 滤波器的非线性相位导致不同频率分量的群延迟不同，造成明显的波形失真和振荡
    \item \textbf{级联效应}：高通和低通滤波器级联使用，相位失真进一步累积
...
\begin{enumerate}
    ...
    \item \textbf{线性相位}：理想滤波器保持线性相位，各频率分量的相对时间关系不变，波形失真较小
...
\textbf{频域分析的重要性：}通过 Q1 的频谱分析，我认识到频域是理解信号特性的关键视角。时域信号可能看起来复杂无规律，但在频域中，其结构往往一目了然。通过对称峰值分析，我成功确定频谱对称轴即频率偏差 $f_d = f_c - \tilde{f_c} \approx 3000$ Hz（而非简单的峰值搜索得到的 3225 Hz），这为后续的解调工作奠定了基础。这让我体会到，\textbf{选择正确的分析域和正确理解频谱特征往往是解决问题的关键}。
...
\textbf{调试与优化：}在实现过程中遇到了许多实际问题，如频率搬移方向错误、滤波器系数计算精度、FFT 结果归一化、频率偏差估计方法等。通过仔细分析中间结果、对比理论值、编写验证程序等方法逐一解决。特别是：
\begin{itemize}
    \item \textbf{频率偏差估计}：发现频谱在 3000 Hz 附近对称分布，通过对称峰值法（2775.20 Hz 和 3225.16 Hz 的中点）得到正确的 $f_d = 3000$ Hz，而非单纯寻找能量峰值（3225 Hz）
    \item \textbf{高通滤波器设计}：采用频谱反转法（Spectral Inversion）代替传统的模拟域变换，实现了高精度的截止频率控制（误差 $< 0.01\%$）
    \item \textbf{方法对比与相关性分析}：Q3 和 Q4 的解调结果相关系数为 0.402。通过振幅归一化验证，发现相关系数对振幅缩放不敏感（归一化前后相关系数相同），这是皮尔逊相关系数的固有性质。0.402 的低相关性反映的是波形\textbf{形状}差异（IIR 滤波器的非线性相位失真 vs 理想滤波器的 Gibbs 振铃），而非振幅差异（Q3 幅度比 Q4 大 28\%）
\end{itemize}
...
本次设计中，原始信号存在频率偏差 $f_d = f_c - \tilde{f_c} \approx 3000$ Hz，这导致解调不当时信号频谱整体偏移。通过 Q1 的对称峰值分析，准确确定了频谱对称轴位置，Q3 和 Q4 都能正确补偿这个偏差。这说明\textbf{准确理解频谱特征和正确的参数估计方法是后续处理的基础}。需要注意的是，频谱中的峰值（如 3225 Hz）反映的是原始信号的能量分布，而非频率偏差本身。
...
\textbf{挑战}：Q3 和 Q4 的解调结果相关系数仅为 -0.028（负相关），远低于预期，表明实现存在严重错误。
...
\textbf{效果验证}：
\begin{itemize}
    \item Q3 和 Q4 的相关系数从 -0.028 提升至 0.402（弱正相关）
    \item 两种方法现在都正确地处理相同的频率范围
    \item 仍存在显著差异（MSE $= 4.99 \times 10^{-2}$，SNR $= -20.76$ dB）主要来自滤波器实现方式的本质不同：
    \begin{itemize}
        \item \textbf{过渡带特性}：渐变过渡带（-48 dB/octave）vs 理想砖墙截止（无过渡带）
        \item \textbf{相位响应}：非线性相位（群延迟随频率变化）vs 线性相位（频域乘法）
        \item \textbf{时域效应}：平滑响应（IIR 递归特性）vs Gibbs 振铃（sinc 函数截断）
        \item \textbf{波形失真}：Q3 的相位失真导致明显振荡，Q4 的振铃效应影响瞬态响应
    \end{itemize}
    \item 两种方法都成功解调出可辨识的音频信号，但波形细节存在明显差异
\end{itemize}
...
\subsection{Butterworth滤波器参数确定原理}

\subsubsection{设计指标}

设计一个滤波器通常需要满足以下指标：
\begin{itemize}
    \item \textbf{通带截止频率} $f_p$：通带边界频率
    \item \textbf{通带最大衰减} $\delta_p$：通带内允许的最大衰减（dB）
    \item \textbf{阻带截止频率} $f_s$：阻带边界频率
    \item \textbf{阻带最小衰减} $\delta_s$：阻带内要求的最小衰减（dB）
\end{itemize}
...
\textbf{本项目参数：}
\begin{itemize}
    \item 高通滤波器：$f_c = 3000.1823$ Hz，$\omega_c = 2 \times 22050 \times \tan(\pi \times 3000.1823 / 22050) \approx 19893.65$ rad/s
...
\subsection{滤波器系数计算推导}

\subsubsection{步骤1：计算模拟滤波器极点}

对于$n$阶Butterworth低通滤波器，极点在复平面上均匀分布：

\begin{equation}
s_k = \omega_c e^{j\theta_k}, \quad \theta_k = \frac{\pi(2k+n-1)}{2n}, \quad k=1,2,\ldots,n
\end{equation}

对于8阶滤波器（$n=8$），选择左半平面的8个极点：

\begin{align}
\theta_k &= \frac{\pi(2k+7)}{16}, \quad k=1,2,\ldots,8 \\
s_k &= \omega_c(\cos\theta_k + j\sin\theta_k)
\end{align}
...
\begin{table}[htbp]
\centering
\caption{8阶Butterworth滤波器极点}
\begin{tabular}{|c|c|c|c|}
\hline
$k$ & $\theta_k$ & $\text{Re}(s_k)$ & $\text{Im}(s_k)$ \\
\hline
...
注意：由于对称性，只需取前4对共轭极点（k=1,2,3,4及其共轭）。

\subsubsection{步骤2：频率预畸变}

将截止频率从$f_c$预畸变到$\omega_c$：
...
\begin{equation}
s_k' = \omega_c \cdot s_k
\end{equation}

\subsubsection{步骤3：双线性变换}

将每个模拟极点$s_k'$转换为数字极点$z_k$，使用双线性变换：

\begin{equation}
s = \frac{2f_s(1-z^{-1})}{1+z^{-1}} \quad \Rightarrow \quad z = \frac{2f_s + s}{2f_s - s}
\end{equation}

对于复数极点$s_k' = \sigma_k + j\omega_k$：

\begin{equation}
z_k = \frac{2f_s + \sigma_k + j\omega_k}{2f_s - \sigma_k - j\omega_k}
\end{equation}

设$T = 1/f_s$，可以简化为：

\begin{align}
z_k &= \frac{2 + \sigma_k T + j\omega_k T}{2 - \sigma_k T - j\omega_k T} \\
    &= \frac{(2 + \sigma_k T) + j\omega_k T}{(2 - \sigma_k T) - j\omega_k T}
\end{align}

利用复数除法：

\begin{equation}
z_k = \frac{[(2 + \sigma_k T) + j\omega_k T][(2 - \sigma_k T) + j\omega_k T]}{(2 - \sigma_k T)^2 + (\omega_k T)^2}
\end{equation}

\subsubsection{步骤4：构造二阶节（SOS）}

每对共轭极点$(s_k, s_k^*)$对应一个二阶节。对于极点$s_k = \sigma + j\omega$：

\textbf{模拟域二阶节：}
\begin{equation}
H_a(s) = \frac{\omega_c^2}{(s-s_k)(s-s_k^*)} = \frac{\omega_c^2}{s^2 - 2\sigma s + (\sigma^2 + \omega^2)}
\end{equation}

\textbf{双线性变换后的分子（前向差分）：}

对于低通滤波器，$s \to (1-z^{-1})/(1+z^{-1})$使得：
\begin{itemize}
    \item $s=0$ 对应 $z=1$ （DC分量）
    \item 分子为 $(1+z^{-1})^2 = 1 + 2z^{-1} + z^{-2}$
\end{itemize}

因此二阶节的分子系数为：
\begin{equation}
b_{\text{section}} = [1, 2, 1]
\end{equation}
...
\begin{align}
(z-z_k)(z-z_k^*) &= z^2 - (z_k+z_k^*)z + z_kz_k^* \\
                  &= z^2 - 2r_k\cos\phi_k \cdot z + r_k^2
\end{align}

除以$z^2$得：
\begin{equation}
1 - 2r_k\cos\phi_k \cdot z^{-1} + r_k^2 z^{-2}
\end{equation}

因此二阶节的分母系数为：
\begin{equation}
a_{\text{section}} = [1, -2r_k\cos\phi_k, r_k^2]
\end{equation}

\subsubsection{步骤5：级联所有二阶节}

对于8阶滤波器，有4个二阶节，需要将它们的系数卷积：

\begin{align}
B(z) &= B_1(z) \cdot B_2(z) \cdot B_3(z) \cdot B_4(z) \\
A(z) &= A_1(z) \cdot A_2(z) \cdot A_3(z) \cdot A_4(z)
\end{align}
...
\begin{align}
p &= p_1 \ast p_2 \\
p[k] &= \sum_{i=0}^{2}\sum_{j=0}^{2} p_1[i] \cdot p_2[j] \cdot \delta[k-i-j]
\end{align}

结果为5项多项式：$[p_0, p_1, p_2, p_3, p_4]$

重复卷积4次，最终得到9项多项式（8阶）。

\subsubsection{步骤6：归一化}

\textbf{归一化$a_0 = 1$：}

\begin{align}
a_i' &= a_i / a_0, \quad i = 0,1,\ldots,8 \\
b_i' &= b_i / a_0, \quad i = 0,1,\ldots,8
\end{align}
...
\begin{equation}
G_{DC} = \frac{\sum_{i=0}^{8} b_i'}{\sum_{i=0}^{8} a_i'}
\end{equation}

最终系数：
\begin{equation}
b_i'' = b_i' \cdot \frac{\sum a_i'}{\sum b_i'}
\end{equation}

\subsubsection{步骤7：频谱反转（高通滤波器）}

对于高通滤波器，先设计镜像频率$f_c' = f_s/2 - f_c$的低通滤波器，然后：
...
\subsubsection{本项目实际计算示例}

\textbf{低通滤波器（$f_c = 4000$ Hz）：}

\begin{enumerate}
    \item 预畸变：$\omega_c = 2 \times 22050 \times \tan(\pi \times 4000/22050) = 26682.17$ rad/s
    \item 计算8个极点，缩放到$\omega_c$
    \item 双线性变换得到数字极点
    \item 构造4个二阶节，级联卷积
    \item 归一化得到最终系数
\end{enumerate}
...
结果（9个系数）：
\begin{align*}
b &= [1.221 \times 10^{-3}, 9.770 \times 10^{-3}, 3.420 \times 10^{-2}, \ldots] \\
a &= [1.000, -2.183, 2.990, -2.531, \ldots]
\end{align*}

\textbf{高通滤波器（$f_c = 3000.1823$ Hz）：}

\begin{enumerate}
    \item 镜像频率：$f_c' = 22050/2 - 3000.1823 = 8024.8177$ Hz
    \item 设计低通滤波器（步骤同上）
    \item 对奇数索引系数取反
\end{enumerate}
...
结果（9个系数）：
\begin{align*}
b &= [1.020 \times 10^{-1}, -8.162 \times 10^{-1}, 2.857, \ldots] \\
a &= [1.000, -3.630, 6.427, -6.942, \ldots]
\end{align*}
...
\begin{itemize}
    \item 阶数：$n = 8$
    \item 截止频率：$f_c = 3000.1823$ Hz（通过对称峰值法确定的频率偏差$f_d$）
    \item 归一化截止频率：$\omega_c = 2\pi f_c / f_s = 0.8549$ rad
...
需要$n$个状态变量$w[n-1], w[n-2], \ldots, w[n-n]$。
...
\begin{itemize}
    ...
    \item \texttt{Q4\_vs\_Q3\_full\_comparison.png} - 全时域波形对比图
    \item \texttt{Q4\_vs\_Q3\_comparison.png} - 局部波形对比图（前2000采样点）
\end{itemize}
...
\lstinputlisting[
    ...
    basicstyle=\ttfamily\scriptsize,
    breaklines=true
]{codes/Q1/src/frequency_estimator.rs}
...
\lstinputlisting[
    ...
    basicstyle=\ttfamily\scriptsize,
    breaklines=true
]{codes/Q1/src/spectrum_visualizer.rs}
...
\lstinputlisting[
    ...
    basicstyle=\ttfamily\scriptsize,
    breaklines=true
]{codes/Q2/src/butterworth_filter.rs}
...
\lstinputlisting[
    ...
    basicstyle=\ttfamily\scriptsize,
    breaklines=true
]{codes/Q2/src/response_visualizer.rs}
